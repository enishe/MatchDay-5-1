      ...
      <form onSubmit={handleRezervim}>
        <div className="booking-grid">
          ...
          <div>
            {/* Lloji i Terrenit */}
            <div className="card">
              <div className="card-title">Lloji i Terrenit (US #1)</div>
              <div style={{ display: 'flex', gap: 10 }}>
                <button type="button" style={s.terrainBtn(terreni === '')}
                  onClick={() => setTerreni('')}>Të gjitha</button>
                <button type="button" style={s.terrainBtn(terreni === 'artificial_grass')}
                  onClick={() => setTerreni('artificial_grass')}>Bar Artificial</button>
                <button type="button" style={s.terrainBtn(terreni === 'indoor_hall')}
                  onClick={() => setTerreni('indoor_hall')}>Sallë Futsali</button>
              </div>
            </div>
            ...
            <div className="card">
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                {fushat.map(f => (
                  <div key={f.id} style={s.fushaCard(parseInt(fushaId) === f.id)}
            ...
            {data && (
              <div className="card">
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7,1fr)', gap: 6 }}>
                  {ORET.map(o => (
                    <button key={o} type="button" style={s.oraBtn(ora === o)}
                      onClick={() => setOra(o)}>{o}</button>
                  ))}
                </div>
              </div>
            )}
            ...
            <div className="card">
              ...
              {fusha && (
                <div style={{ background: '#e8f5e9', border: '1px solid #a5d6a7', borderRadius: 8, padding: 12, marginBottom: 14 }}>
                  <div style={{ fontSize: 11, color: '#2e7d32', marginBottom: 4, fontWeight: 500 }}>SMART SPLIT (US #2)</div>
              ...
              <div style={{ background: 'var(--bg-secondary)', borderRadius: 8, padding: 12, marginBottom: 16 }}>
                <label style={{ display: 'flex', alignItems: 'center', gap: 10, cursor: 'pointer' }}>
                  ...
                  <div>
                    <div style={{ fontSize: 13, fontWeight: 500 }}>Patika me Qira (US #3)</div>
